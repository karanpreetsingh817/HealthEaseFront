...
const MyComponent = ({name,email,age, experience, specialization, description,appointmentFee, qualification,password,confirmPassword,image}) => {
  ...
  const handleVerify=async(el)=>{
    el.preventDefault();
     ...
     try {
      ...
      const result = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}doctor/signup`, {name,email,age, experience, specialization, description,appointmentFee, qualification,password,confirmPassword,image,OTP:otp},
       {
        headers: {

...
const Doctor = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}doctor/admin/${Cookie.get("doctorId")}`, {
                headers: {
            ...
            setAppointmentFee(data.result.appointmentFee)
            setExperience(data.result.experience)
            setDoctor(data.result);
            setProfile(data.result.profileImg.url)
        }
        fetchData();
    },[]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            toast.error('🦄 Failed Doctors Updation', {
                position: "top-center",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
                ...
                <div className="-mx-4 flex flex-wrap rounded-md">
                    <div className="w-full px-4 lg:w-1/3">
                        <div className="wow fadeInUp relative mx-auto mb-12  max-w-[500px] text-center lg:m-0 flex justify-center">
                            <Image src={profile} alt="Doctor-profile-IMG" width={250} height={350}  className="rounded-full mb-8 items-top mt-4"  />
                        </div>
